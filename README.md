...
Before connecting a persistent database, review the external [Configuration repository] at a recorded revision and disable SQL initialization with the version-appropriate `spring.sql.init.mode=never`. HSQLDB schema scripts drop tables; MySQL scripts create databases/tables and the seed scripts insert demo records. These are development fixtures, not migrations; do not run them against existing user data.
