...
Owner: runtime contract; audited for COG-174. This is a local demonstration runbook, not a production deployment procedure. Start with [setup](setup.md); use [checks](checks.md) for gates and [validation](validation.md) for historical COG-172 outcomes; [COG-174 runtime](cog-174-runtime.md) records the current static audit. No deployed environment or provider account was exercised by this documentation thread.
...
[Compose](../../docker-compose.yml) exposes Zipkin at 9411, Admin at 9090, Grafana at host 3030 and Prometheus at host 9091. [Prometheus configuration](../../docker/prometheus/prometheus.yml) targets Docker DNS names for Gateway, Customers, Visits and Vets, plus itself. It does not scrape GenAI, Config, Discovery or Admin. The mixed host-Java launcher does not supply an alternative scrape configuration, so do not claim its monitoring path works without verifying container-to-host resolution. Grafana's [configuration](../../docker/grafana/grafana.ini) enables anonymous access with the `Admin` role; these sample interfaces must remain inside the intended local boundary. Compose port mappings do not bind exclusively to loopback. Do not expose them on public infrastructure as part of an agent task.

The checked-in Grafana panel titled `Visit Created` queries `petclinic_visit_seconds_count{method="read"}`, while `VisitResource.create` handles writes and `read` handles reads. Treat that panel as mislabeled, not creation evidence. Dashboard import, metric compatibility and actual scraping require a runtime check; the JSON parsing check establishes only syntax.
...
- [Chaos scripts](../../scripts/chaos/README.md) deliberately inject latency, exceptions, memory pressure and application termination. They send state-changing actuator requests to local domain services. Run only in an explicitly authorized isolated experiment; record restoration steps and verify normal behavior afterward. The `attacks_disable.json` and `watcher_disable.json` payloads disable their respective flags; the helper accepts them even though usage omits `attacks_disable`. Only the exception and memory payloads explicitly set `level: 5`; the chaos README's every-fifth-request statement is not a verified contract for all assaults.
- [tagImages.sh](../../scripts/tagImages.sh) changes local image tags using `REPOSITORY_PREFIX` and `VERSION`; [pushImages.sh](../../scripts/pushImages.sh) publishes all eight images to a registry. Neither script enables fail-fast behavior or validates its variables: an earlier image operation can fail while the final operation succeeds. Their final exit code alone does not establish success for all eight images, that tests passed, or that the target/tag is correct. Confirm intended repository and immutable version before an authorized release. Do not execute publishing or set `container.build.extraarg=--push` as validation.
