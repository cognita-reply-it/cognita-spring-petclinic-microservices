...
Owner: validation thread for COG-172; future changes follow the owner of the affected build or test contract. Run commands from repository root unless noted. [Validation evidence](validation.md) records actual executions; the commands below describe available checks, not successful results. [Setup](setup.md) owns prerequisites and supported start paths.
...
[Java CI with Maven](../../.github/workflows/maven-build.yml) triggers on pushes to `main` and pull requests targeting `main`. It uses `ubuntu-latest`, checkout v4, setup-java v4, Java 17/adopt and Maven dependency caching. COG-172 preserves these choices and the existing package gate, switches to the already pinned Maven 3.9.8 wrapper and adds the offline documentation guard. There is no configured artifact upload, release publication, registry push or deploy stage. Branch protection requirements are GitHub settings and cannot be inferred from YAML.
...
The documentation guard checks simple inline Markdown links and reference definitions outside fenced code, including linked images. It excludes URL reachability, anchors, HTML links, code snippets and semantic accuracy. It checks env **names**, not their correctness against every Spring relaxed-binding variant or whether a value is secret; review the source/template manually. It reports required files missing rather than silently passing an incomplete handoff. No network, build outputs or secret values are needed.
